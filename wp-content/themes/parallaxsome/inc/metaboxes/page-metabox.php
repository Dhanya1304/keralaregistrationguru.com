 ...
 */
 ...
 add_action( 'add_meta_boxes', 'parallaxsome_page_meta_options' );
 
 if( ! function_exists( 'parallaxsome_page_meta_options' ) ):
 function  parallaxsome_page_meta_options() {
    add_meta_box(
                'parallaxsome_page_meta', // $id
                esc_html__( 'Page Options', 'parallaxsome' ), // $title
                'parallaxsome_page_meta_callback', // $callback
                'page', // $page
                'normal', // $context
                'high'
            ); // $priority
 ...
 $parallaxsome_page_sidebar_options = array(
 ...
 */
if( ! function_exists( 'parallaxsome_page_meta_callback' ) ):
	function parallaxsome_page_meta_callback() {
		global $post, $parallaxsome_page_sidebar_options;
		wp_nonce_field( basename( __FILE__ ), 'parallaxsome_page_meta_nonce' );
?>
		<div class="ps-meta-container clearfix">
			...
			<div class="ps-metabox-content-wrapper">
				...
				<!-- Info tab content -->
				<div class="ps-single-meta" id="ps-info-content">
				...
				<div class="ps-single-meta" id="ps-sidebar-content">
					...
					<div class="ps-meta-options-wrap">
						<div class="buttonset">
							<?php
			                   	foreach ( $parallaxsome_page_sidebar_options as $field ) {
			                    	$parallaxsome_page_sidebar = get_post_meta( $post->ID, 'ps_page_sidebar_layout', true );
			                ?>
			                    	<input type="radio" id="<?php echo esc_attr( $field['id'] ); ?>" value="<?php echo esc_attr($field['value']); ?>" name="ps_page_sidebar_layout" <?php checked( $field['value'], $parallaxsome_page_sidebar ); if( empty( $parallaxsome_page_sidebar ) && $field['value'] == 'default_layout' ){ echo "checked='checked'";}  ?> />
 ...
 * @since 1.0.0
 */
add_action( 'save_page', 'parallaxsome_save_page_meta' );

if( ! function_exists( 'parallaxsome_save_page_meta' ) ):

function parallaxsome_save_page_meta( $post_id ) {

    global $post, $parallaxsome_page_sidebar_options;

    // Verify the nonce before proceeding.
    if ( !isset( $_POST[ 'parallaxsome_page_meta_nonce' ] ) || !wp_verify_nonce( wp_unslash($_POST['parallaxsome_page_meta_nonce'] ), basename( __FILE__ ) ) )
        return;
    ...
    foreach ( $parallaxsome_page_sidebar_options as $field ) {  
        ...
        $old = get_page_meta( $post_id, 'ps_page_sidebar_layout', true ); 
        $new = sanitize_text_field( wp_unslash($_POST['ps_page_sidebar_layout'] ));
        if ( $new && $new != $old ) {  
            update_page_meta ( $post_id, 'ps_page_sidebar_layout', $new );  
        } elseif ( '' == $new && $old ) {  
            delete_page_meta( $post_id,'ps_page_sidebar_layout', $old );  
        }
    } // end foreach
}
endif;  