...
function clean_journal_get_social_icons(){
	if( ( !$output = get_transient( 'clean_journal_social_icons' ) ) ) {
		...
		$pre_def_social_icons 	=	clean_journal_get_social_icons_list();
				...
				$output .= '<a class="login-btn btn-login" href="'.get_site_url().'/ML'.'">Malayalam</a>';
                
                if ( is_user_logged_in() ) {
					$output .= '<a class="login-btn btn-login" href="'.wp_logout_url( home_url() ).'">Logout</a>';
				}else{
					$output .= '<a class="login-btn btn-login" href="/login">Login</a>';
				}
                $output .= '<a class="signup-btn btn-login" href="/signup">Sign Up</a>';
